...
namespace Sonarr.Api.V3.ManualImport
{
    ...
    public class ManualImportController : Controller
    {
        ...
        public object ReprocessItems([FromBody] List<ManualImportReprocessResource> items)
        {
            ...
            {
                var processedItem = _manualImportService.ReprocessItem(item.Path, item.DownloadId, item.SeriesId, item.SeasonNumber, item.EpisodeIds ?? new List<int>(), item.ReleaseGroup, item.Quality, item.QualityManuallySelected == true, item.Languages, item.IndexerFlags, item.ReleaseType);
